...
import { useGroupAssetInfo } from '@/contract/assets/assets';
import { useNavigate } from 'react-router-dom';
import { getViewportSize, type AssetViewProps } from './AssetView';


export function AssetViewImage({ identifier }: AssetViewProps) {
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (!groupAssetInfo?.isInGroup) {
      return;
    }
    
    const toNext = location.pathname.replace(
      `/asset/${identifier}/view`,
      `/asset/${groupAssetInfo.nextAsset}/view`,
    );

    const toPrev = location.pathname.replace(
      `/asset/${identifier}/view`,
      `/asset/${groupAssetInfo.previousAsset}/view`,
    );
  ...
  const { data: groupAssetInfo } = useGroupAssetInfo(identifier);
